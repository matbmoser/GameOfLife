...
spec = [
    ('tablero', Tablero),               # a simple scalar field
    ('pauseExect', boolean),
    ('finish',boolean),
    ('epochs',int32),
...
class Tablero():
    ...
    def calculate_neighbours(self, x, y):
        ...
        return self.gameState[(x - 1) % self.nxC, (x - 1)  % self.nyC] + \
                self.gameState[(x)     % self.nxC, (x - 1)  % self.nyC] + \
                self.gameState[(x + 1) % self.nxC, (x - 1)  % self.nyC] + \
                self.gameState[(x - 1) % self.nxC, (x)      % self.nyC] + \
                self.gameState[(x + 1) % self.nxC, (x)      % self.nyC] + \
                self.gameState[(x - 1) % self.nxC, (x + 1)  % self.nyC] + \
                self.gameState[(x)     % self.nxC, (x + 1)  % self.nyC] + \
                self.gameState[(x + 1) % self.nxC, (x + 1)  % self.nyC]
...
class ParalellGameOfLife():
    ...
    def updateGameState(self):
        ...
        for y in range(0, self.tablero.nxC): 
            ...
            #### Paralizar------------------
                tmpGameState = self.vertical(y,tmpGameState, dimCW, dimHW)
            #### ------------------        
        #### ------------------  
    ...
    @jit(parallel=True)
    def vertical(self, y, tmpGameState, dimCW, dimHW):
