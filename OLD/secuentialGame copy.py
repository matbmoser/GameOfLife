...
def startSecuentialGame(w, h, nx, ny, epoch=0):
    pygame.init()
...
def updateSecuentialGameState():
    global gameState, nxC, tmpGame, dimCW, dimCH
    ...
    for y in range(0, nxC): 
        ...
        for x in range (0, nyC):
            ...
            if not pauseExect:    
                ...
                num_neighbours = gameState[(x + 1) % nxC, (y + 1)  % nyC]+\
...
if __name__ == '__main__':
    ...
    start = timer()
    startSecuentialGame(w=width, h=height, nx=nxC, ny=nyC, epoch=epochs)
    end = timer()
